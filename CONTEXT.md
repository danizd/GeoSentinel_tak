...
**MediaMTX** — Servicio `mediamtx` del compose (`bluenviron/mediamtx:1.21.0`, imagen multi-arch, ADR 0012): recibe el directo de una cámara por **RTSP** (8554) y lo re-sirve a los clientes TAK; el 8888 publica el mismo stream por **HLS** para comprobarlo en el navegador. **Publicar exige credenciales** (`takvideo` + `MEDIAMTX_PASSWORD`, en `.env`, no en `mediamtx.yml`); **leer es anónimo** desde el ADR 0014 — WinTAK y ATAK no envían usuario/contraseña en un stream RTSP (VLC sí), así que el 8554 expuesto solo protege la publicación y la **ruta del stream hace de secreto** del feed.

**Ruta del stream** — Nombre con el que se publica un directo en MediaMTX (`MOVILGALICIA`, `webcam`…): es el *Stream Path* de OpenTAK ICU en el móvil y el *Path* en la herramienta Vídeo de WinTAK. Cualquier ruta se crea al publicar (`paths: all_others:`), sin declararlas una a una.

**OpenTAK ICU** — App Android **independiente** (no un plugin dentro de ATAK, aunque el vídeo se vea luego en ATAK/WinTAK) que publica la cámara del móvil como stream **RTSP** hacia MediaMTX. Se instala desde el APK de sus *releases* (`brian7704/OpenTAK_ICU`) y se configura en *Streaming Settings*: *Stream Protocol* `rtsp`, *Stream Address* (IP del servidor), *Stream Port* `8554`, *Stream Path* (la ruta del stream, **una sola palabra sin espacios**), **usuario y contraseña** de MediaMTX y **TCP** ON. Su *onboarding* propio pide los permisos (cámara, micrófono y ubicación; la ubicación *Allow all the time* es lo que habilita su CoT — sin ella publica igual, pero no emite nada al mapa). Sustituye al plugin **TAK ICU** del TAK Product Center, descartado en el móvil de la demo porque *ATAK → Settings → Plugins → TAK ICU → Install/Enable* entra en un **bucle de permisos** que no deja avanzar; es el mismo código base del que el ADR 0013 copió la estructura de `<__video>`, de ahí que su puntero valga igual.
