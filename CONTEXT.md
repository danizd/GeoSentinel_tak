...
**El vídeo NO viaja por CoT** — CoT transporta a lo sumo un puntero (la URL del feed, en un evento `b-i-v` con `<__video>`), nunca los fotogramas: el vídeo va por RTSP entre el publicador y MediaMTX. Sin ese evento CoT se ve el vídeo en el reproductor, pero **no aparece icono de cámara en el mapa**; nada del proyecto lo emite todavía.
